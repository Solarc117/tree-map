...
;(async () => {
  let movies
  try {
    movies = await d3.json(
      'https://cdn.freecodecamp.org/testable-projects-fcc/data/tree_map/movie-data.json'
    )
  } catch (error) {
    console.error(error)
    alert('Could not load tree map data')
  }

  const hierarchy = d3
    .hierarchy(movies, movie => movie.children)
    ...
    function optimizeLines(title, lines = []) {
      const words = Array.isArray(title)
        ...
        : title
            .split(' ')
            .filter(s => s)
            .map(word => (checkIfFits(word) ? word : word.slice(0, 3) + '.'))
      let currentLine = []
      while (
        words.length > 0 &&
  ...
  cells.forEach(cell => {
    ...
    function showTooltip(target) {
      function addCommas(int) {
        ...
        return subArrs.reduce((acc, curVal) => acc.concat(',', curVal)).join('')
      }
